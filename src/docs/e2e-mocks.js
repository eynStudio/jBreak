(function(ng){
    var module=ng.module('e2e-mocks',[ 'ngMockE2E']);
    ...
    module.run(function ($httpBackend, $timeout, $q) {
        "use strict";
    //    $httpBackend.whenGET(new RegExp('views\/.*')).passThrough();

        $httpBackend.whenGET(/\/api\/phones\?page=\w+$/).respond(function(method, url, params){
            var re = /.*\/api\/phones\?page=(\w+)/;
            var p = parseInt(url.replace(re, '$1'), 10);

            return [200,{
                Total:phones.length,
                Items:phones.slice((p-1)*5,p*5)}
            ];
        });
        $httpBackend.whenPOST('/api/phones').respond(function(method, url, data, headers){
            var p=angular.fromJson(data).page;
            return [200,{
                Total:phones.length,
                Items: phones.slice((p - 1) * 10, p * 10)
            }, {}
            ];
        });
        $httpBackend.whenGET('/api/IbZd?bq=pp').respond(function (method, url, params) {
            //var d = $q.defer();
            //$timeout(
            //    function() {
            //        d.resolve([200, [
